/* KlooBR · controle de localização (país) na nav.
   Protótipo: a seleção é lembrada em localStorage. Ao chegar, um palpite
   silencioso (idioma/fuso do navegador) define o país; o usuário pode abrir o
   modal para detectar com precisão (geolocation) ou buscar e escolher na lista. */
(function () {
  'use strict';
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var dlg = document.querySelector('[data-ctry-dialog]');
    var listEl = document.querySelector('[data-ctry-list]');
    if (!dlg || !listEl) return;

    var flagEls = Array.prototype.slice.call(document.querySelectorAll('[data-ctry-flag]'));
    var nameEls = Array.prototype.slice.call(document.querySelectorAll('[data-ctry-name]'));
    ...
    document.querySelectorAll('[data-ctry-open]').forEach(function (b) { b.addEventListener('click', openDlg); });
    var cl = document.querySelector('[data-ctry-close]');
    ...
    /* Estado inicial: escolha salva, senão Brasil (padrão). A detecção por
       geolocation acontece só quando o usuário clica em "Detectar". */
